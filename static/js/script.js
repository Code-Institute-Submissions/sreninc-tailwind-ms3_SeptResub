...
function starRating(number) {
    ...
    for (let i = 0; i < 5; i++) {
        x = i;
        if (i <= number) {
            let full = `<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline text-yellow-400" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" onclick="starRating('{{ x }}')" id="star-{{ x }}">
